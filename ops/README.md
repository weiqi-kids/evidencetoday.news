...
9. **去重名冊保留期分兩級**（2026-09-07）：`data/processed-sources.json` 的條目，`outputFile` 有值（已成稿）保留 90 天、為 `null`（未成稿）只保留 14 天，鍵在 `data/news-automation-config.json` 的 `dedup.retentionDays` / `dedup.unwrittenRetentionDays`，規則寫在 news SELECT_BLOCK 與 `docs/news_sop.md`「四、去重機制」。由來：2026-09-06 那輪素材池全空、選題階段連跑都沒跑到——當天撈回來的候選全在 90 天名冊裡。未成稿多半只是當天分數差一點，鎖三個月等於讓系統看不到自己的第二次機會。**不要改回單一 90 天。**
