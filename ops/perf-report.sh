...
6.【供 Slack】另用 Write 寫一份**精簡摘要**到「$SLACK_MD」（純文字、≤14 行、給 Slack 看）：第一行 \`:bar_chart: *站況＋建議 $DATE*\`；接著「數據一句話（曝光/點擊/排名/索引 X/Y 與前期比）」；然後「🤖 自動優化近 14 天已處理 N 項」一行；再來「🙋 待你決策」3–5 條最重要的 bullet（每條一行、可執行）。不要把整份報告塞進去。
