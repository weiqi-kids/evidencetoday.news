...
case "$TYPE" in
  news)
    SELECT_BLOCK="撰寫趨勢文章。請依本 repo 的 AGENTS.md「撰寫趨勢文章」與 docs/news_sop.md 執行完整管線：Phase 1 用 WebSearch 跑 data/news-automation-config.json 的查詢建素材池（某組 allowed_domains 被封鎖回 400 整組全滅就記錄略過）；去重比對 data/processed-sources.json（已處理跳過，素材池空則靜默結束，印「無新草稿」）；列既有檔用 ls/Glob 勿 Read 目錄；Phase 2 執行 node scripts/audience-insights.mjs 讀三桶（topicCandidates 併素材池標 internal-demand、writingDirectives 注入撰文、siteOptimizations 收進 run summary 不自動改既有頁）；五維度加權選題。**門檻制（重要）：只有加權分數 ≥8.0 的高把握選題才成篇；當天沒有夠強的選題就什麼都別留、印「無新草稿」靜默結束。** 同主題分組、最高分者優先，本輪至多 1 份高把握工單。**寧可整週零產出，也不要為發而發**——全站真實索引率僅約 11%（189 頁卡在 Discovered–not indexed），Google 對新 URL 的態度是價值判斷，持續灌入低需求頁面會直接稀釋網域權重。**標題鐵則：\`titleDisplay\` 必須是讀者會實際打進搜尋框的問句或具體疑問**（例：「更年期荷爾蒙療法的黑框警語拿掉了，代表可以放心用嗎？」），**嚴禁「健康雷達 YYYY-MM-DD」這類日報流水句型、嚴禁把期刊名或研究設計當標題主體**；並確認前 18 字單獨看仍讀得通（social-meta 的 shortTitle 會截斷）。" ;;
  articles)
