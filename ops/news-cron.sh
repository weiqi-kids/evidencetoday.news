...
【子代理模型｜硬性省成本】凡用 Agent 工具派子代理一律顯式帶 model 參數，**嚴禁用預設模型（會落到 opus、最貴）**：撰寫與審核委員會一律 model='sonnet'（比照本 SOP 設計 Sonnet x n）；僅純機械性檢查（連結驗 200、檔名格式）可用 model='haiku'。
...
Phase 3【務必真並行】每份工單派一個 sub-agent（Agent 工具，subagent_type=general-purpose，model='sonnet'）撰文，**在同一則訊息內一次發起全部 Agent 達成真正並行**，不要逐一發起等前一個結束；每個 sub-agent 只負責一篇、回傳該篇 markdown，所有數字與連結只能來自素材原始來源，禁止編造；
...
Phase 5/6 動態審核委員會：每篇文章的每位審核角色都派一個 sub-agent（Agent 工具，model='sonnet'），**同一則訊息內一次發起該輪全部審核 Agent 並行**，收齊回饋後由主 agent 彙整修稿再進下一輪；連續 3 輪未收斂才判草稿；審核必查「references 至少 1 筆含可點 url」；
Phase 7 發布前先跑 `pnpm check:news` 與 `pnpm content:audit` 預檢（兩者皆須 pass）。content:audit 會擋下 AI 味；命中就改到 pass。守統一去 AI 味禁用句型（8 類，權威清單見 docs/content-guide.md「鐵則」，各生成端一致）：① 禁「不是X，而是Y」下定義與「不僅…更/還」「不只是…而是/更是」「並非…而是」排比（對比用「是Y，而非X」）；② 禁「值得注意的是/值得一提的是/換句話說」；③ 禁空泛收束（總的來說/綜上所述/總而言之/歸根結底/整體而言）；④ 禁「真正的問題/關鍵是…」拔高與「隨著…的發展/普及」「在…的今天」開場公式；⑤ 禁「至關重要/不可或缺/舉足輕重」；⑥ 禁模糊引用（研究顯示/有研究指出/專家認為/學者認為/普遍認為）——要嘛附具體可點來源/具名歸因、要嘛軟化為主編判讀、要嘛不寫（禁編造來源）；⑦ 禁破折號（——）下定義；⑧ 禁模板化第一人稱開場（以「我」起句、我一直覺得/老實講/朋友最常問我/最近有讀者/我發現/我觀察），開頭第一句直接給具體價值。正向：長短句交錯、每段換開法、每段至少一個具體事實、台灣口語、容許口語瑕疵。全部通過者更新 processed-sources.json 後 commit + push main；未收斂者設 draft:true 開 PR 等人工。（content:audit 現已是 deploy 的硬性 gate，未過會擋部署。）
