...
//
// 【evidencetoday 特化】本檔＝ new-astro-site skill 統一引擎 templates/check-content.mjs 的本站副本；
//   上方核心跨站共用、不改；本站把原 scripts/audit-ai-tone.mjs 的 evidence 特化規則 port 到下方 SITE_* 擴充點
//   （2026-07-21 統一，取代自有 AI 腔 gate；結構/來源類 gate check:myths / check:news 另檔保留）。
//
// 兩級判定：
...
//
// 【evidencetoday port 自舊 audit-ai-tone.mjs（只補「核心沒有、evidence 舊 gate 有」者，核心已涵蓋者不重複）】
const SITE_ERROR_TELLS = [
  // 模糊引用·醫療特化（vague-reference）：核心已有 研究顯示/有研究指出/多項研究指出/國外研究發現/
  //   臨床研究指出/專家認為/學者認為/有文獻(表示|指出)；下列為 evidence 舊 gate 額外的醫療腔模糊引用。
  ["模糊引用（醫療特化，無出處）", /(某研究指出|文獻回顧|研究者觀點|摘自臨床生化教學文獻)/],
  // AI 感句型（ai-phrase，全文攔截）：evidence 舊 gate 有、核心未涵蓋者。
  ["「這件事值得說清楚」", /這件事值得說清楚/],
  ["「我一直覺得」（正文）", /我一直覺得/],
  ["「老實講/說」（正文，非僅開頭）", /老實(講|說)/],
  ["「有人說」（模糊歸因）", /有人說/],
  // 作者假借第一人稱自證經驗（我觀察／我在臨床）是 AI 量產文最常見口氣；舊 gate 由開頭擴為全文攔截
  //   （見 2026-07-21 SEO/AEO 稽核：大量出現在第 2、3 段）。讀者/引述語氣的「我發現／我最近」不在此列。
  ["第一人稱自證經驗（我觀察/我在臨床）", /(我觀察|我在臨床)/],
];
const SITE_WARN_LAYERS = { 詞彙: [], 句式: [], 結構: [], 語氣: [] };
// evidence 舊 gate 的模板化開頭「朋友最常問我/朋友常問我」；核心 BANNED_OPENINGS 已含 我…／老實講／最近有讀者。
const SITE_BANNED_OPENINGS = [/^朋友(最常|常)問我/];
// 舊 gate 的 raw-enum（meta-analysis/rct/observational/animal/in-vitro）刻意「不 port」：舊 gate 本就只警告不擋，
//   且多數命中是 references 的合法 type: schema 值與正文研究類型用詞（前台已由 src/utils/evidence-labels.ts 中文化），
//   放進 WARN 層反而可能湊足「跨 ≥3 層」誤升級為 ERROR。故略過（見交付回報）。
...
for (const re of SITE_BANNED_OPENINGS) BANNED_OPENINGS.push(re);
