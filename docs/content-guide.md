...
> **2026-09-05 更新：這件事現在有建置時的自動處理，但規則沒有廢除。**
> `src/utils/rehype-unpublished-links.mjs`（接在 `astro.config.mjs` 的 `rehypePlugins`）
> 會在建置時把指向未公開頁面的站內連結降級成純文字，**目標一上線、下次建置就自動變回連結**。
> 所以寫作時可以放心互相連結，不必先算兩篇的 `publishDate` 先後。
>
> 但**仍要跑 `pnpm check:schedule`**：外掛只是讓部署不會炸，它不會告訴你「這條連結目前是失效的」。
> 守門會逐條列出哪一天會爆，你才能決定是調整排程日期、還是接受它暫時以純文字呈現。
> 那個守門的判定在 2026-09-05 修過一次——原本漏掉「兩篇都還沒發布、但來源比目標早」的組合，
> 當時有 18 條地雷躲過檢查，見 `pitfalls.md`。
