...
- **🚫 禁止模板化第一人稱開頭（最高優先，YMYL 致命傷）**：不准用「我一直覺得…」「我最近…」「老實講…」「朋友最常問我…」「我發現…」「我觀察…」這類固定人設開場白。內文開頭第一句**直接給具體價值**——一個數據、一個明確主張、一個具體情境，而非作者的感想或鋪陳。原因：全站曾有 42% 文章用同一套開場白，在健康（YMYL）類別會被 Google 判為 AI 量產內容而拒絕索引。**每篇開頭都要不一樣。**
- **🚫 禁止 AI 感連接句型**：不准用「不是…而是」（含「並非…而是」「不只是…更是」）「換句話說」「真正的問題是」「這件事值得說清楚」等公式化轉折。要表達對比，用「是Y，而非X」或正面直述。
- **🚫 禁止模糊引用**：不准用「研究顯示」「文獻回顧」「有研究指出」「專家認為」「學者認為」等空泛開頭。每個 claim 要嘛**具名歸因**（年份／作者／期刊／機構，且對應 frontmatter `references`），要嘛**軟化為主編判讀**（「目前證據顯示…」「就現有資料看…」）。禁止為了過 gate 而編造來源。
- **`pnpm content:audit` 是硬性 gate（強制擋）**：`banned-opening`（模板化開頭）、`ai-phrase`（AI 感句型）、`vague-reference`（模糊引用）任一命中即 `exit 1`、CI 擋下，**不受 warning/strict 模式影響**（2026-06-23 全站清零後鎖定防回歸）。`raw-enum` 仍為警告（多為 references 的合法 `type:` 值）。發布前務必跑到全綠。
  - **2026-07-21 擴掃**：`我觀察`、`我在臨床` 這兩個第一人稱口氣原本只在「正文第一句」被 `banned-opening` 擋，實際卻常出現在文中第 2、3 段（一次清掉 24 處）。現已加入 `ai-phrase` **全文攔截**（命中任一行即擋）。注意：只掃這兩個作者假借語氣；讀者/引述語氣的 `我發現`（FAQ 問句）、`我最近`（示範對醫師說的話、自我檢核清單）是合法的，**不在攔截範圍**，寫作時可正常使用。
