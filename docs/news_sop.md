...
- 素材未產出文章時仍會記錄為已處理（避免反覆嘗試無效來源），但**保留期比照下表分兩級**

| 條目類型 | 判準 | 保留期 | 設定鍵 |
|---|---|---|---|
| 已成稿 | `outputFile` 有值 | 90 天 | `dedup.retentionDays` |
| 未成稿 | `outputFile` 為 `null` | **14 天** | `dedup.unwrittenRetentionDays` |

超過各自保留期的條目，每次執行時自動清除。

⚠️ **為什麼分兩級**（2026-09-07 改，不要改回單一 90 天）：未成稿的素材多半是「當天分數差一點」，而研究與法規題材會隨時間發展——兩週後可能有後續、有爭議、有官方回應，那時分數就夠了。用 90 天鎖住它，等於讓系統永遠看不到自己的第二次機會。2026-09-06 那輪就是這樣：素材池撈回來的東西全在 90 天名單裡，**一筆新素材都沒有，選題階段連跑都沒跑到**。已成稿的維持 90 天，那是真的要防重複撰文。
...
- **🚫 禁止模板化第一人稱開頭（YMYL 致命傷）**：不准用「我一直覺得 / 我最近 / 老實講 / 朋友最常問我 / 我發現 / 我觀察」這類固定人設開場。開頭第一句直接給具體價值（數據、明確主張、具體情境），每篇都要不一樣。
- **🚫 禁止 AI 感句型**：「不是…而是 / 不只是…更是 / 換句話說 / 真正的問題是 / 這件事值得說清楚」。發布前必跑 `pnpm content:audit`，命中 `ai-phrase` 一律改掉再發（健康類這些指紋會被 Google 判為 AI 量產而拒絕索引）。
- 禁止聳動用語：「震驚」「突破性」「革命性」
