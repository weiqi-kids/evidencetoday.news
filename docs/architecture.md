...
| Schema.org JSON-LD（內容頁） | 每個頁面的 `<head>` | Article+MedicalWebPage（multi-type，帶 `@id …#article`、`mainEntityOfPage`、`inLanguage: zh-Hant-TW`）、FAQPage、VideoObject、BreadcrumbList；文章/成分頁帶 `image`（取 frontmatter `coverImage`，缺則省略）；文章/闢謠頁 author 透過 `buildPerson()` 帶 `@id`（`…/#person`）與 sameAs；publisher→`ORG_ID`、isPartOf→`WEBSITE_ID`；`references` 透過 `buildCitations()` 輸出 schema.org `citation`（CreativeWork：name/url/publisher/date/genre/doi）；成分解析頁 author/reviewedBy/publisher 皆為 `ORG_ID` 參照，含 `lastReviewed`、`medicalAudience`（Patient）；myths 頁額外輸出 ClaimReview（`buildClaimReview`）、BreadcrumbList（含「迷思查證」中間層）與 FAQPage（當 `faq` 非空時，與可見 Q&A 一致）；健康專題頁（`/topics/`）輸出 CollectionPage + ItemList + FAQPage（**ItemList 涵蓋該頁列出的所有類型**，不只文章）；**6 個 collection 列表頁（`/articles/` `/myths/` `/ingredients/` `/news/` `/podcasts/`）皆輸出 CollectionPage（內嵌 ItemList，逐筆列出該頁所有內容 URL）＋ BreadcrumbList**，builder 為 `schema-org.ts` 的 `buildCollectionPage()` / `buildBreadcrumbList()`，經 `List.astro` 的 `head` slot 注入（`/news/` 直呼 `Base`）；作者頁輸出 Person（`buildPerson`，**羅揚為牙醫學學歷背景，非執業牙醫師，無 `hasCredential` 執照宣稱**）。所有內容型 JSON-LD 的 `inLanguage` 統一為 `zh-Hant-TW`（含 podcasts/videos） |
| Open Graph + Twitter Card | `src/layouts/Base.astro` | og:title, og:description, og:type, twitter:card；大多數頁面 og:image 用 `/og-static/*.png`（分類圖手動上傳；作者頁專屬 `author-luo-yang.png` 由 `scripts/generate-author-og.ts` 經 satori→sharp 一次性生成，文案/版面變更時重跑、commit 入 git）。**闢謠（myths）例外**：每篇有專屬逐篇 OG，build 時由 `src/pages/og/myths/[slug].png.ts`（1200×630，判讀色帶＋標題＋證據強度）與 `[slug]-square.png.ts`（1080×1080「儲存圖卡」，供下載後直接轉傳）用 `src/utils/og-template.ts` 的 `generateMythOgSvg` / `generateMythSquareCardSvg` 動態生成，不進 git；`src/utils/social-meta.mjs` 的 `contentSocial('myths', …)` 會把 og:image 指到 `/og/myths/{slug}.png?v={updatedDate 時間戳}`（內容更新自動換版本、免快取殘留）。闢謠單篇頁的分享區塊（`src/pages/myths/[slug].astro` `.share-block`）秀出這張 OG 圖預覽＋「分享到 LINE」（`social-plugins.line.me/lineit/share`）＋「下載圖卡」（存圖卡 png），取代舊版直接把 `MythSquareCardVisual` 當分享卡片嵌在頁面裡的做法（該元件仍用於 `/myths/` 列表頁的卡片視覺，未移除）。 |
...
| sitemap.xml | `@astrojs/sitemap` + `serialize` | 每篇內容頁輸出 `lastmod`（`updatedDate ?? publishDate`，來源 `scripts/lib/content-dates.mjs` 掃 frontmatter）；`/admin`、`/tags/*`（noindex,follow）、`/search`、`/404` 以 `filter` 排除（noindex 頁不列入 sitemap）；舊 slug 轉址不進 sitemap |
...
| LINE 官方帳號推薦後端訊號 | `src/pages/reader-index.json.ts`（build 期產出候選池）＋ `ReadingBeacon.astro`（掛 articles 單篇頁，`sendBeacon` 送 `hi.evidencetoday.news`） | 供外部服務 `my-line-bot-customer`／appinews-reader 取內容與真讀訊號；與 GA4/`ReadingEngagement` 職責不同、終點不同，並存不合併。契約正本在 `/root/appinews-reader/contracts/`，詳見 **`docs/playbooks/reader-index-and-beacon-contract.md`** |
