...
- **每份工單 = 一個 sub-agent**（`Agent` 工具，`subagent_type=general-purpose`），**同一則訊息內一次發起全部**達成真並行，每個只寫一篇、回傳該篇 markdown。
...
news 管線的品質把關有四道，但**只有一道是確定性的**：

| 把關 | 性質 | 能擋什麼 | 擋不到什麼 |
|---|---|---|---|
| `pnpm build`（Zod `newsSchema`） | ✅ 確定性 | 必填欄位缺、型別錯、tags 含 `/` | **references / url / sourceUrl 都是 `.optional()`** → 沒來源連結照樣 build 過 |
| `content:audit`（audit-ai-tone） | ⚠️ warning-only | AI 感句型、模糊引用提示 | 不阻擋，也不檢查「每筆 reference 有無 url」 |
| CI 連結檢查（deploy.yml） | ✅ 確定性 | HTML 裡**已存在**的連結是否死連 | 純文字來源「根本沒連結」→ 無連結可檢，直接過 |
| Phase 5/6 AI 審核委員會 | ❌ 非確定性 | 靠 LLM 自覺 | 會漏；零建議就自動 push |
