...
├─ pnpm build             ← 內含 check-design + check-content（去 AI 味守門）+ astro build
...
`deploy.yml` 的 build job 依序跑會擋部署的內容 gate（任一失敗 → build 中斷 → 不部署）：
1. `pnpm check:news`：每篇非 draft 的 news 須有可點來源連結。
2. `pnpm check:myths`（2026-07-12 加入 deploy）：已發佈 myths 篇數須等於 `scripts/check-myth-quality.mjs` 的 `EXPECTED_PUBLISHED_COUNT`、8 個固定 body 區塊齊全、藍/紅 reasoningCards 各 ≥2 點、references URL ≥2、固定醫療提醒句等。**增刪已發佈闢謠時務必同步改該常數**，否則此 gate 會擋部署。
3. 去 AI 味守門（`scripts/check-content.mjs`）**已併入 `pnpm build`**（`check-design && check-content && astro build`），不再是獨立 step。2026-07-21 統一為 new-astro-site skill 的引擎（取代舊 `audit-ai-tone.mjs`）。它預設 **grandfather**（只掃相對 origin/main 的變動檔）：直推 main 時 CI 掃 0 檔，**真正的擋點是「提交前 `pnpm build`」**（本地手動與 cron ops gate 都會跑到、違規當場擋）與 PR 時的 `content-audit.yml`。規範見 `CLAUDE.md` 硬規則 7a 與 `docs/content-guide.md`。
