...
## 實作紀錄（P1 部分完成，2026-07-21）

**第 8、9 項完成**（第 7、10 項需人力／決策，未動）：

- ✅ **補齊 27 篇零引用文章的 `references`**（清單第 8 項）：以「研究子代理找源 → 驗證子代理逐條 WebSearch 覆核」兩段式流程產出，共約 97 筆引用、每篇 2–5 筆。**抗幻覺紀律**：每個 URL 均須在實際 WebSearch 結果中出現；驗證階段抓到並剔除／修正數筆問題（如 EFSA 4455 實際「否定」DHA 記憶宣稱、屬誤引；astaxanthin 一筆被誤標為 review 實為單組研究；JAAD 一筆 article ID 疑似杜撰）。以官方機構（NCCIH/ODS/FDA/WHO/衛福部/國健署 等）為骨幹，佐以可辨識的地標研究（DPP、LEAP、EAT、ISSN 肌酸立場聲明、BMJ 咖啡傘型分析）。前台引用列表與 schema.org `citation` 皆已生效，103/103 文章現皆有引用。
- ✅ **改寫 24 處第一人稱 AI 味＋擴掃 gate**（清單第 9 項，見上一則 commit）。

**尚未處理**：第 7 項（掛名醫療審閱者，需先決定人選）、第 10 項（成分/保健文末就近顯示利益揭露），及 P2 全部。
