...
- **正文寫 `P<0.001` 這種統計符號會讓整站 build 失敗**（2026-09-12）：MDX 把「`<` 後面接數字」當成 JSX 開標籤，錯誤訊息是
  ...
  **frontmatter 不受影響**（那是 YAML），只有正文會炸；同一份稿 YAML 裡的 `P<0.008` 沒事、正文的 `P<0.001` 會死，
  很容易誤判成「檔案沒問題啊」。
  **修法**：正文一律寫成 `&lt;`（本站 MDX 本來就在用 `<a href="#ref-1">` 這類 HTML，實體編碼不衝突），
  或改成 `P < 0.001`（`<` 與數字之間有空格就不會被當標籤）。
  **產內容時的預防**：交稿後、跑 build 前先掃一次 `grep -n '<[0-9]' <檔案>` 的**正文區**，比等 build 紅了再逐個修快得多。
- **「台灣怎麼管」這種句子最容易過時，也最容易被當成常識直接抄**（2026-09-22，一次查出三個）：
  - 乳薊（水飛薊）被當成「台灣護肝保健食品最常見的成分」寫了好幾篇，但食藥署平臺列它「未確認安全性尚不得使用之原料」，合法產品是**藥品**。
  - 純化的納豆激酶同樣列為不得作食品原料（納豆菌本身可以）；成分頁卻寫它「被歸類為食品或保健食品成分」，還附了兩家廠商的產品頁當來源。
  - 「台灣唯一合法的褪黑激素是亞眠靚」在 115 年 3 月泰豪眠發證後就錯了，五篇文章沒人知道。
  規則：**凡是寫到某成分在台灣的法規身分，一律現查**食品原料整合查詢平臺的開放資料
  （`https://data.fda.gov.tw/data/opendata/export/4/csv`，整份下載後 grep 中英文名）與西藥許可證，
  並在來源標題註明查詢日。不要沿用站上其他文章的說法——那正是錯誤擴散的途徑。
  全站盤點用 `pnpm audit:regulation`（`scripts/audit-ingredient-regulation.mjs`）：列出平臺把成分或其某部位列為不得使用、
  成分頁卻沒提到的頁面。第一次跑就又抓出鋸棕櫚、東革阿里、纈草三頁。
- **參考文獻的 PMID 指向完全無關的論文，而標題、作者、期刊都寫得像模像樣**（2026-09-22）：
  維生素 C 那篇把虛構的作者與期刊（「Zhao Y 2022, Hematol Rep」）套在一個真實 PMID 上；用 NCBI 比對全站 1,649 個 PMID 後，
  又抓出 8 筆——基因轉染研究被當成維生素轉運研究、養護機構物理治療被當成 VITAL 試驗、鈣與骨折世代研究被當成硫辛酸研究。
  **其中好幾筆的 PMID 只差 1**（9516450 寫成 9516451、17068980 寫成 17068979），像是憑印象寫了一個相近的數字。
  人工讀稿完全看不出來，只有點進連結才會發現。另外還抓到一筆「兩篇論文拼成一筆」的條目（標題與期刊都用斜線並列）。
  全站盤點用 `pnpm audit:pmids`（`scripts/audit-pmids.mjs`）。它刻意保守：只比對「姓 縮寫,」開頭的條目，
  標題裡的資料期間（「2012-2021」「from 2003」）不當成出版年——所以報出來的幾乎都是真的，但沒報的不代表都對。
- **MDX 裡留一條 HTML 註解 `<!-- ... -->`，全站部署就紅燈**（2026-09-17，紅了五小時）：MDX 不支援 HTML 註解，
  `<` 後面接 `!` 會被當成 JSX 起手式，錯誤訊息是 ``Unexpected character `!` (U+0021) before name``。
  那條註解是撰寫工具的「這裡要放示意圖」佔位符（`<!-- type:diagram ... -->`），圖沒生成、佔位符卻留在檔案裡。
  跟上一條 `P<0.001` 是同一類：**都是 `<` 開頭、都只在正文炸、都是一個檔案拖垮全站**。
  真的需要註解用 `{/* ... */}`。交稿前掃一次：`grep -n '<!--\|<[0-9]' <檔案>`。
...
- **序列化偷換內容**：`check-boilerplate` 用 `Array.join()` 比對陣列欄位，物件陣列
  ...
  確認序列化後的字串真的代表原本的內容。
- **「恆 exit 0」的盤點工具，壞掉時跟正常時長得一模一樣**（2026-09-21）：`check:content:all` 與
  `check:spec:all` 設計上永遠 exit 0（人工普查用）。同一天查出它們各有一個讓結果失真的 bug，兩個都沒人發現過：
  1. **Windows 上掃 0 檔**。腳本用 `execSync("git ls-files 'src/**/*.md'")`，而 `execSync` 在 Windows 走
     `cmd.exe`，單引號是字面字元，git 收到含引號的 pathspec 就比對不到任何檔案，輸出「無變動的內容檔」。
     同一道指令貼到 Git Bash 手動跑是好的，所以怎麼測都測不出來。修法：pathspec 不帶引號，過濾交給 JS。
  2. **`check:content:all` 把 ERROR 整批丟掉，然後印「無 AI 味 ERROR」**。程式寫的是 `if (errors.length && !ALL)`，
     `--all` 時整個區塊跳過、直接落到最後那行成功訊息。修好當天的真實讀數是全站數十處 ERROR——
     而修之前它回報的是 0。**盤點工具回報一個它根本沒在看的結論，比沒有工具更糟**，因為會有人據此說「全站是乾淨的」
     （我自己就說了）。
  規則：**不擋的檢查更需要驗證它真的有在看東西**。寫完先確認兩件事——掃描檔數不是 0、刻意放一個違規它報得出來。
- **有一條發文路徑完全不經過任何內容守門，而且這件事被誤診了兩次**（2026-09-21 查清）：
  `check:content`／`check:spec` 比對的是 `merge-base origin/main HEAD`，設計上的擋點是「提交前的本機 `pnpm build`」；
  CI 直推 main 時 origin/main 就是 HEAD，掃 0 檔，這點 `deploy.yml` 的註解有寫。問題是**有一條路徑根本沒有本機**：
  repo 外的工具透過 GitHub API 直接提交文章（commit 訊息是 `content: <標題>`、圖片是 `images: <slug> N.jpg`）。
  對它而言 CI 是唯一的關卡，而那道關卡是空的。2026-09-07 與 09-17 兩次有人注意到「新增檔繞過守門」，
  都記成「根因未查」，因為一直往營運主機的 cron 產線找——**其實看 `git log` 的 author 與訊息格式就能分辨是哪條路徑**。
  回測過去 8 天：該路徑進來的文章有三分之二未達七月標準（幾乎都是正文站內連結 0 條），另有二十多處 AI 味 ERROR，
  包含虛構的第一人稱臨床經驗。
  已做的修正：兩支腳本支援 `GATE_BASE_SHA`，`deploy.yml` 新增 `content-gate` job 在 push 事件帶入
  `github.event.before`，檢查「這次 push 帶進來的檔案」，未過就亮紅燈並發 Slack。**刻意不擋部署**——
  每小時的 schedule 觸發沒有 before 可比、會照常放行，放進 build 只會變成「擋一小時後自己解除」。
  真正的約束要做在上游那支工具裡。
