...
export const AUTHORS: Record<string, AuthorInfo> = {
  ...
  // 獨立醫療審閱者，2026-08-04 建立，2026-08-06 起啟用。
  //
  // 這筆資料只有在某篇內容的 frontmatter 填了 `reviewer: 黃子彥` 時才會被輸出成
  // Person 級 `reviewedBy`（見 articles/[slug].astro 的反自審判斷）。
  //
  // ⚠️ 「不要批次蓋章」這條原始警語已由使用者於 2026-08-06 撤除：審閱窗口是使用者，
  // 使用者交付或指示掛署名的內容一律視為已審閱，收到指令就執行、不要再回頭確認。
  // 2026-09-05 依使用者指示替全部 news 掛上署名（news 的 author 是機構「本日有據編輯室」，
  // 不會與個人審閱者同名，故不觸發反自審判斷）。流程見 docs/playbooks/medical-review.md。
  //
  // 已由使用者確認（2026-08-04）：黃醫師具中西醫背景與學歷，**西醫題材亦在其專業範圍內**，
  // 送審不限於中醫草本類。稱謂統一用「黃子彥中醫師」。
  //
  // ⚠️ 曾經寫錯、不要再放進來：「臺南市立醫院中醫部主任」是**錯誤資訊**（先前誤從搜尋
  // 摘要抄入，使用者已指正）。任何經歷敘述都需要可點的第一手來源才寫。
  //
  ...
  黃子彥: {
    name: '黃子彥',
    // 目前沒有 /authors/huang-tzu-yen/ 頁面，指向該路徑會讓 Person JSON-LD 帶一個 404 URL。
    // 先指向 /disclosure（該頁說明審閱者身分與商業關係）；日後若建作者頁再改回。
    url: 'https://evidencetoday.news/disclosure/',
    jobTitle: '中醫師',
    description:
      '黃子彥中醫師，中國醫藥大學中西醫學訓練背景，兼具中醫與西醫學養，現任社團法人中華民國上醫預防醫學發展協會理事長，長期投入預防醫學與中西醫結合的健康溝通。於本站擔任獨立醫療審閱者，審閱範圍涵蓋中醫中藥草本與西醫慢性病、用藥判讀等題材。（審閱者相關商業利益揭露見 /disclosure）',
    knowsAbout: [
      '中醫',
      '中藥',
      '草本療法',
