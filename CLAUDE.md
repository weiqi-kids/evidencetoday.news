...
## ⏰ 到期提醒：news 發文頻率重新評估（**2026-08-15**，每次 session 檢查日期）

**每次 session 若今天 ≥ 2026-08-15，主動提醒使用者做這件事**（未到期則不必提，避免噪音）。日期由使用者於 2026-08-06 指定，從原訂的約 09-01 提前。
...
**⚠️ 08-15 是期中檢查，不是結案。**原訂 09-01 的理由是「72 篇全發完＋累積約一週數據」；08-15 時 08-05→08-22 那批只發到一半（約 40/72），最晚上線的幾篇連 Google 都還沒爬完，**最後幾天的稿子必然數據不足，不能當成「表現差」的證據**。08-15 該做的是看**趨勢方向**（曝光還在不在成長、索引率有沒有繼續往上），方向明確才動結構；數據還在爬升中就維持現狀、把完整結論留到全批發完後再下。

**到期要做的評估**：跑 `pnpm perf` 與 `pnpm index:coverage`，對照下表判斷 news 是否該減量：

| 指標 | 2026-08-05 基準 | 判讀 |
|---|---|---|
| news 每頁曝光 | 2.8（articles 28.1） | 若仍 <5，減量的理由成立。**但只計算已上線 ≥7 天的稿**，剛發的拉低平均是必然，不是訊號 |
| news 未索引比例 | 33/85（39%） | 若未改善，代表產出未被 Google 採納。同上，新稿本來就還沒被索引，要排除 |
| 全站曝光趨勢 | 4,381／28 天且仍在加速 | 若仍在成長，優先「別亂動」——這是 08-15 期中檢查的**主要**判準 |
