...
**目前卡在兩件事，未解決前不要寫入任何 `reviewer` 欄位：**
1. **身分未確認**：網路搜尋找不到「黃子彥中醫師」，只找到黃子玶（悅兒親子中醫診所．中醫兒科）、黃彥鈞（太初中醫）、黃子洛（香港）。需使用者提供正確姓名與可驗證連結（診所簡介頁／醫師公會頁），才能填 `src/data/authors.ts` 的 `hasCredential`。**嚴禁憑推測填真實醫師的憑證。**
2. **審閱範圍需相符**：中醫師憑證與本站大量西醫主題（statin、疫苗、Cochrane 統合分析、青光眼指南）領域不符。建議優先審**草本／中藥／保健食品**類（銀杏、紅景天、薑黃、南非醉茄、紅豆水、轉骨湯、秋葵水等），西醫疾病指南另尋審閱者或不掛。

機制本身已就緒：`content.schemas.ts` 有 `reviewer` 欄、`articles/[slug].astro:88-91` 已實作「審閱者≠作者才輸出 Person 級 `reviewedBy`」。只差 `AUTHORS` registry 新增一筆。
