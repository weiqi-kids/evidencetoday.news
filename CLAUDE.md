...
1. **第二批 20 篇送審進度？** 清單見 `docs/medical-review-queue.md`「第二批」，重心已從「保健品×藥物交互作用」換成**就醫時機判讀與症狀鑑別**（誤判會延誤診斷，且用得上中西醫雙背景）。審完一篇填一篇。
2. **有沒有哪篇涉及上一生物醫學的產品線？** `/disclosure` 第 3 點承諾「涉及審閱者任職公司產品或明確利益的頁面不掛署名」。第二批選篇已刻意避開純保健品品項評比（如 `nmn-taiwan-legal-buy`），但**我不知道該公司實際產品線**，需使用者判斷是否還要再剔除。⚠️「臺南市立醫院中醫部主任」是**錯誤資訊**，已移除，不要再寫回去。
