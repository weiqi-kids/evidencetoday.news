...
2026-07-21 稽核把「找一位有證照的獨立醫療審閱者」列為 YMYL 權威**最大的單一槓桿**（目前 103 篇中只有 1 篇有 `reviewer`，且是作者本人＝schema.org 反自審模式，不計分）。使用者已指定由**黃醫師（中醫師）**擔任，並同意**階段性審核**（審完一篇填一篇，不一次蓋章）。

**身分已確認**（2026-08-04）：黃子彥，中醫師，中國醫藥大學中西醫學訓練背景，現任社團法人中華民國上醫預防醫學發展協會理事長，另任上一生物醫學研發長。`src/data/authors.ts` 的 `AUTHORS` 已建好資料，但**刻意尚未啟用**——目前站上沒有任何一篇掛 `reviewer`。

**第一批 20 篇已於 2026-08-05 完成審閱並掛上署名**（`reviewer: 黃子彥`，前台顯示「審閱：黃子彥中醫師」連到 `/disclosure`，並輸出 Person 級 `reviewedBy` 含 `hasCredential`）。`/disclosure` 的「醫療審閱者」段落已揭露其任職上一生物醫學的利益衝突，並承諾逐篇制與產品迴避。

**每次 session 請主動追問使用者這兩件事：**
1. **第二批 20 篇的回覆進度？** 2026-08-06 已送審（送審文件 `docs/medical-review-batch2-send.md`，可直接轉貼；進度表在 `docs/medical-review-queue.md`）。重心是**就醫時機判讀與症狀鑑別**（誤判會延誤診斷，用得上中西醫雙背景）。**審完一篇填一篇 `reviewer: 黃子彥`，嚴禁批次蓋章。**掛署名前的 check-content 阻擋已於送審時預先清掉（autonomic 與 melatonin-dosage 各修一句），現在 20 篇全綠。
2. **第三批要不要納入 2026-08-23→08-28 那 6 篇新文？** 其中 4 篇涉及用藥判斷（阿斯匹靈×魚油、甲狀腺素間隔、抗生素×益生菌、日本藥妝管制成分），正是中西醫雙背景的守備範圍。

⚠️ 兩件已澄清、不要再重複問或寫錯：①**上一生物醫學是研發公司、無自有產品**（使用者 2026-08-06 確認），故無直接商業利益衝突，`/disclosure` 第 3 點保留為保險條款即可，選篇不必再迴避保健品題材。②「臺南市立醫院中醫部主任」是**錯誤資訊**，已移除，不要再寫回去。
