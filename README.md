...
### 7. TopNav 設計鎖定（高度敏感元件）

`src/components/blocks/TopNav.astro` 的字體、字級與文字位置經多輪量測對齊。動到之前先讀 git log（特別是 `94428c5`、`0099cdd`、`1ce471a` 三個 commit）。常見錯誤：

- **「nav 文字突然靠上」**：父層用 `align-items: stretch` 把 `<li>` 撐到 navbar 全高，但忘了讓內層 `<a>` 也 `height: 100%`。anchor 自然高度只有文字高度（約 24px），會貼到 li 頂端
- **「nav 字體跟 logo 不一樣」**：nav link 必須跟 `.topnav__brand-zh` 和 h1 一致 — `font-family: var(--font-serif)` + `font-size: clamp(1.05rem, 1.8vw, 1.45rem)` + `font-weight: 700` + `line-height: 1.2`
- **「nav 字底沒貼到 navbar 底邊」**：當前 spec 是「字底端距 navbar bottom border 固定 20px」。padding 公式：`padding-bottom = target_distance - half_leading`，其中 `half_leading = (line-height − font-size) / 2`。改了 font-size / line-height / target_distance 任一項都要重算
- **跨斷點對齊**：nav 在 `≥900px` 顯示，必須在 `@900px` 與 `@1280px` 兩個邊界都量。用 linear clamp 公式 `clamp(minRem, calc(Apx - Bvw), maxRem)`，A 是 @900 的目標 px、B 是斜率
- **驗證方法**：改完用 Playwright + `getComputedStyle` 量測，肉眼看不出 1–2px 誤差。量測 script 範本見 git commit `1ce471a` 的描述
